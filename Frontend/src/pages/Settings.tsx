    ...
    <RootDiv style={{}}>
      <div className="w-full max-w-[1400px] py-10 space-y-10">
        ...
        >
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <SettingItem label={t("settings.accent_color")} desc={t("settings.accent_desc")}>
            ...
            <SettingItem label={t("settings.language")} desc={t("settings.language_desc")}>
              <Button onClick={handleLangToggle} variant="secondary" className="min-w-[80px]">
                {lang === "vi" ? "Tiếng Việt" : "English"}
              </Button>
            </SettingItem>

            <SettingItem label={t("settings.compact_mode")} desc={t("settings.compact_desc")}>
        ...
        <Section
          ...
          icon={<Image size={18} className="text-vie-primary" />}
        >
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <SettingItem
        ...
        <Section
          title={t("settings.vie_pro_section")}
          icon={<Crown size={18} className="text-amber-400" />}
        >
          <div className="space-y-4">
            <SettingItem
              label={t("settings.vie_pro_status")}
              desc={t("settings.vie_pro_status_desc")}
            >
              <div className="flex flex-col items-end gap-2">
                ...
                >
                  {licenseTier === "pro"
                    ? t("settings.vie_pro_badge_pro")
                    : t("settings.vie_pro_badge_free")}
                </span>
            ...
            <SettingItem
              label={t("settings.vie_pro_activate_label")}
              desc={t("settings.vie_pro_activate_desc")}
            >
          ...
          <div className="space-y-4">
              ...
              <div className="flex items-center gap-5">
                ...
                <div className="space-y-1">
                  <h4 className="text-sm font-bold tracking-wider uppercase text-white">
                    {t("settings.auto_admin")}
                  </h4>
                  <p className="text-xs text-white/40 font-medium">
                    {t("settings.auto_admin_desc")}
                  </p>
                </div>
              </div>
            ...
            <div className="flex items-center justify-between p-5 rounded-2xl bg-[#09090b]/40 border border-white/5 hover:border-white/10 transition-colors shadow-inner">
              <div className="flex items-center gap-5">
                ...
                <div className="space-y-1">
                  <h4 className="text-sm font-bold tracking-wider uppercase text-white">
                    {t("settings.system_tray")}
                  </h4>
                  <p className="text-xs text-white/40 font-medium">
                    {t("settings.system_tray_desc")}
                  </p>
                </div>
              </div>
            ...
            <div className="flex items-center justify-between p-5 rounded-2xl bg-[#09090b]/40 border border-white/5 hover:border-white/10 transition-colors shadow-inner">
              <div className="flex items-center gap-5">
                ...
                <div className="space-y-1">
                  <h4 className="text-sm font-bold tracking-wider uppercase text-white">
                    {t("settings.unlock_fps_tray")}
                  </h4>
                  <p className="text-xs text-white/40 font-medium">
                    {t("settings.unlock_fps_tray_desc")}
                  </p>
                </div>
              </div>
              <Button
        ...
        >
          <div className="flex items-center gap-4">
            ...
            <p className="text-xs text-vie-text-dim">{t("settings.clear_backup_desc")}</p>
          </div>
        </Section>
      </div>
      ...
      <Modal open={deleteModalOpen} onClose={() => setDeleteModalOpen(false)}>
        <div className="p-6 text-center space-y-4">
          ...
          <p className="text-sm text-vie-text-muted">{t("settings.modal_delete_desc")}</p>
...
const SettingItem = ({ label, desc, children }: { label: string; desc: string; children: any }) => (
  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-5 rounded-2xl bg-[#09090b]/40 border border-white/5 focus-within:border-cyan-500/30 transition-colors group">
    <div className="space-y-1">
      <h4 className="text-sm font-bold tracking-wider uppercase text-white/90 group-hover:text-white transition-colors">
        {label}
      </h4>
