...
const cleanupsData: CleanupItem[] = [
  ...
  {
    id: "prefetch",
    labelKey: "clean.prefetch",
    script: "",
  },
  {
    id: "update",
    labelKey: "clean.update",
    script: "",
  },
  {
    id: "recycle",
    labelKey: "clean.recycle",
    script: "",
    dangerous: true,
  },
  {
  ...
  {
    id: "shader",
    labelKey: "clean.shader",
    script: "",
  },
  {
    id: "browser",
    labelKey: "clean.browser",
    script: "",
    dangerous: true,
  },
  {
    id: "updatebackup",
    labelKey: "clean.updatebackup",
    script: "",
    dangerous: true,
  },
]
                                ...
                                ${
                                  item.dangerous
                                    ? selected.includes(item.id)
                                      ...
                                      : "bg-[#09090b]/80 border-rose-500/10 hover:border-rose-500/30"
                                    : selected.includes(item.id)
                                      ? "bg-cyan-500/10 border-cyan-500/30 shadow-[0_0_20px_rgba(34,211,238,0.1)]"
                                      : "bg-[#09090b]/80 border-white/5 hover:border-white/10"
                                }
                            `}
            ...
            >
              <div className="flex items-center gap-5">
                  ...
                  className={`p-3 rounded-xl transition-colors duration-300 ${
                    item.dangerous
                      ? selected.includes(item.id)
                        ...
                        : "bg-white/5 text-rose-400/50 group-hover:text-rose-400/80"
                      : selected.includes(item.id)
                        ? "bg-cyan-500/20 text-cyan-400"
                        : "bg-white/5 text-white/40 group-hover:text-cyan-400/80"
                  }`}
                >
                  {item.dangerous ? (
                    <AlertTriangle
                      size={22}
                      className={selected.includes(item.id) ? "animate-pulse" : ""}
                    />
                  ) : (
                    <Trash2 size={22} />
                  )}
                ...
                <div className="space-y-1">
                  <h3
                    className={`font-bold tracking-wide uppercase text-sm transition-colors duration-300 ${selected.includes(item.id) ? "text-white" : "text-white/60"}`}
                  >
                    {t(item.labelKey)}
                  </h3>
                  {item.dangerous && (
