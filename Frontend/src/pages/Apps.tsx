...
export default function Apps() {
  ...
  useEffect(() => {
    ...
    const unsub2 = onIpc({
      ...
      listener: () => {
        ...
        if (errors > 0) {
          toast.warning(
            `${t("apps.install_complete")} (${errors} ${errors === 1 ? "error" : "errors"})`,
            { autoClose: 4000 },
          )
        } else {
  ...
  const toggleBloat = (id: string) => {
    setBloatSelected((prev) => (prev.includes(id) ? prev.filter((x) => x !== id) : [...prev, id]))
  }
  ...
  return (
    <PageShell
      ...
      subtitle={
        isVi
          ? "Kho ứng dụng & dọn dẹp bloatware all-in-one"
          : "App Store & Bloatware Cleanup all-in-one"
      }
    >
...
function TabButton({
  ...
  return (
    ...
    >
      ...
      {label}
      <span
        className={`px-1.5 py-0.5 rounded-md text-[10px] ${active ? "bg-white/10" : "bg-white/5"}`}
      >
        {count}
      </span>
    </button>
  ...
  return (
    ...
    >
      <div className="flex items-start gap-3">
        {/* Icon */}
        <div
          className={`w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0 overflow-hidden ${isSelected ? "bg-emerald-500/20" : "bg-white/5 border border-white/10"}`}
        >
          {app.icon && !imgError ? (
            <img
              src={app.icon}
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2">
            <h3
              className={`font-bold text-sm tracking-wide truncate ${isSelected ? "text-white" : "text-white/80 group-hover:text-white"}`}
            >
              {app.name}
            </h3>
            {app.warning && <AlertTriangle size={12} className="text-amber-400 flex-shrink-0" />}
          </div>
          <p className="text-[11px] text-white/40 truncate mt-0.5">{app.info}</p>
          <div className="flex items-center gap-2 mt-2">
            <span
              className={`px-2 py-0.5 rounded-md text-[9px] font-bold uppercase tracking-widest ${catColor.bg} ${catColor.text} ${catColor.border} border`}
            >
              {app.category}
            </span>
          </div>
          ...
          >
            ...
            {isRemoving
              ...
              : isVi
                ? "Trảm Hệ Thống"
                : "Remove"}{" "}
            {bloatSelected.length > 0 ? `(${bloatSelected.length})` : ""}
          </Button>
        </div>
      </div>
            ...
            >
              <div className="flex items-start justify-between gap-4">
                <div className="space-y-1">
                  <h3
                    className={`font-bold tracking-wider text-sm transition-colors ${isActive ? "text-white" : "text-white/80 group-hover:text-white"}`}
                  >
                    {appItem.label}
                  </h3>
      ...
      <div className="flex items-center justify-between px-4 py-3 border-b border-white/10">
        ...
        <div className="flex items-center gap-2">
          {!isInstalling && queue.length > 0 && (
            <button
              onClick={onClear}
              className="text-white/30 hover:text-white text-xs uppercase font-bold tracking-wider"
            >
              {isVi ? "Xóa" : "Clear"}
            </button>
          )}
