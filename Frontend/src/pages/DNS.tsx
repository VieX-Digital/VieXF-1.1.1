    ...
    <PageShell title="Đổi DNS Siêu Tốc" subtitle="Vượt rào, giảm ping, max speed">
      <div className="space-y-8">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            <div className="text-right hidden md:block border-r border-white/10 pr-4">
              <p className="text-[10px] uppercase font-bold text-white/40 tracking-widest mb-0.5">
                DNS Hiện Tại
              </p>
              <p className="text-xs font-black text-cyan-400 uppercase tracking-widest">
                {currentDns === "dhcp"
                  ? "Mặc Định (DHCP)"
                  : dnsProviders.find((d) => d.id === currentDns)?.label || "Custom"}
              </p>
            </div>
            <Button
              variant="ghost"
              className="px-5 py-2 rounded-xl text-xs font-bold tracking-wider uppercase border border-white/10 hover:bg-white/10 hover:text-white text-white/60 transition-all"
              onClick={resetDns}
              disabled={applying}
            >
              Khôi Phục
            ...
            return (
              ...
              >
                <div className="flex gap-5">
                  <div
                    className={`p-3.5 rounded-xl transition-all duration-300 ${isActive ? "bg-cyan-500/20 text-cyan-400 shadow-[0_0_15px_rgba(34,211,238,0.3)]" : "bg-white/5 border border-white/10 text-white/40 group-hover:border-cyan-500/30 group-hover:text-cyan-400 group-hover:bg-cyan-500/10"}`}
                  >
                    <Globe size={26} />
                  </div>
                  <div className="space-y-1">
                    <h3
                      className={`font-bold tracking-wider uppercase text-sm transition-colors duration-300 ${isActive ? "text-white" : "text-white/80 group-hover:text-white"}`}
                    >
                      {dns.label}
                    </h3>
                    <p className="text-xs text-white/40 uppercase tracking-wide">
                      {t(`dns.providers.${dns.id}`)}
                    </p>
                    <div className="flex gap-2 mt-3">
                      <code className="px-2 py-1 rounded-md bg-black/60 border border-white/10 text-xs text-cyan-400/80 font-mono tracking-widest">
                        {dns.primary}
                      </code>
                      <code className="px-2 py-1 rounded-md bg-black/60 border border-white/10 text-xs text-cyan-400/80 font-mono tracking-widest">
                        {dns.secondary}
        ...
        <div className="p-6 rounded-2xl border border-white/5 bg-[#09090b]/80 backdrop-blur-xl flex flex-col md:flex-row md:items-center gap-5 shadow-[0_8px_30px_rgba(0,0,0,0.5)] transition-all hover:border-white/10">
          ...
          <div className="space-y-1">
            <h3 className="font-bold tracking-wider uppercase text-sm text-white">
              Xoá Bộ Nhớ Đệm DNS (Flush DNS)
            </h3>
            <p className="text-xs text-white/40">
              Thanh lọc cache mạng, sửa lỗi không truy cập được web do xung đột DNS cũ.
            </p>
          </div>
          <Button
            variant="ghost"
            className="md:ml-auto px-5 py-2.5 rounded-xl text-xs font-bold tracking-wider uppercase border border-purple-500/30 hover:bg-purple-500/20 hover:text-purple-300 text-purple-400 bg-purple-500/10 transition-all shadow-lg"
            onClick={flushDns}
          >
            Bơm Ngay
          </Button>
