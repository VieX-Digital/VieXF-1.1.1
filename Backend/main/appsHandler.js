...
export const setupAppsHandlers = () => {
  ...
  ipcMain.handle("apps:install", async (event, payload) => {
    ...
    try {
      ...
      hasWinget =
        checkResult.success && checkResult.output && checkResult.output.trim().startsWith("v")
    ...
    for (const app of apps) {
      ...
      sendProgress(
        ...
        name,
        "installing",
        hasWinget ? `Installing via winget...` : `Opening download link...`,
      )
        ...
        try {
          ...
          if (result.success) {
            ...
            if (
              ...
              output.includes("No applicable update found")
            ) {
